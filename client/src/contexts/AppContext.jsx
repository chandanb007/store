...
import * as adminOrderService from "../services/admin/orderService.js";
...
export const AppProvider = ({ children }) => {
  ...
  const [currentUser, setCurrentUser] = useState(() => {
    ...
    return stored ? JSON.parse(stored) : []; // Logged in as customer by default for smooth demo
  });
 ...
 const loadPublicProducts = async (filters) => {
  ...
  useEffect(() => {
    ...
    root.style.setProperty(
      "--font-serif-custom",
      `"${themeConfig.serifFont}", Georgia, serif`,
    );
  ...
  const toggleWishlist = (productId) => {
    ...
    setUsers((prev) =>
      prev.map((u) =>
        u.id === currentUser.id ? { ...u, wishlist: updatedWishlist } : u,
      ),
    );
    setCurrentUser((prev) =>
      prev ? { ...prev, wishlist: updatedWishlist } : null,
    );
  };
  ...
  const forgotPassword = (email) => {
    ...
    return {
      success: false,
      message: "No registered user matches this email address.",
    };
  };
  ...
  const updateThemeConfig = (newConfig) => {
    ...
    addNotification(
      "success",
      "Theme & appearance config updated successfully!",
    );
  };
  //-----------Refined code Starts ----------------------------------//
  // Cart Actions
  ...
  const getOrders = async (filters) => {
    try {
      ...
      if (currentUser.role === 'ADMIN') {
        response = await adminOrderService.getOrders(filters);
      } else {
         response = await orderService.getUser();
      }  
     if (response.status === 200) {
